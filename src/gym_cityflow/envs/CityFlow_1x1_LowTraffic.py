...
class CityFlow_1x1_LowTraffic(gym.Env):
    ...
    def __init__(self):
        ...
        self.steps_per_episode = 200 #100
        self.current_step = 0
        ...
        self.action_space = spaces.Box(low=np.array([3]),high=np.array([10]),dtype=np.float32)
    ...
    def step(self, action):
        # print("Action: ", action)
        # print("Target= ", self.target_steps, " | steps until target=", self.steps_since_change, "   | total: ", self.current_step)

        # round float action number?
        
        if(self.current_step == 0):
            self.target_steps = action
            # print("currently time 0 | target = ", self.target_steps)
            
        # Maybe try continuously changing
        if((self.steps_since_change >= self.target_steps) == 0):
            self.steps_since_change = 0
            self.target_steps = action
            # print("Boom new target: ", self.target_steps)
            if(self.current_light == 8):
                self.current_light = 0
            else:
